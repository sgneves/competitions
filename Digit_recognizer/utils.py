...
def create_models(create_fun, create_params):
    ...
    create_params : dict
        Dictionary with parameters names as keys and respective values. Must have exactly one list
        or 1-D NumPy array that corresponds to the variable parameter. All other parameters are kept
        constant.
...
def keras_fit_eval(model, fit_params, verbose=True):
    """
    Fits a Keras model to the train set and evaluates its accuracies and fitting time.
    ...
    # Fit the model
    start = time()
    history = model.fit(**fit_params).history
    fit_time = time() - start
...
def plot_models_metrics(create_fun, create_params, fit_eval_fun, fit_eval_params, fig_params={},
    ...
    create_fun : function
        Function to use to create the models.
    create_params : dict
        Parameters to pass to create_fun. Must have exactly one list or 1-D NumPy array that
        corresponds to the variable parameter. All other parameters are kept constant.
...
def keras_plot_histories(create_fun, create_params, fit_params, var_param, metric, legend=None,
                         fig_params={}):
    """
    Creates and fits the models and plots their metric histories.
    ...
    create_fun : function
        Function to use to create the models.
    create_params : dict
        Parameters to pass to create_fun.
    fit_params : dict
        Parameters to pass to the fit method of the models.
    var_param : tuple
        (name, value) of the variable parameter.
    metric : str
    ...
    param_type, param_name, param_vals = var_param
    ...
    # Create and fit the models and plot their metric histories
    plt.figure(**fig_params)

    for i, val in enumerate(param_vals):

        # Check if it is a parameter to pass to the create or fit functions
        if param_type == 'create':
            create_var_param = {param_name: val}
            fit_var_param = {}
        elif param_type == 'fit':
            create_var_param = {}
            fit_var_param = {param_name: val}
        else:
            raise ValueError("Variable parameter must be a create or fit parameter")

        # Create the model
        model = create_fun(**create_params, **create_var_param)

        # Fit the model
        start = time()
        history = model.fit(**fit_params, **fit_var_param).history
        fit_time = time() - start

        # Plot the metric
        plt.plot(history[metric])

        args = (legend[i], metric.capitalize(), history[metric][-1], fit_time)
        print('Value: {}; {}: {:.2f}; Fitting time: {:.2f}s'.format(*args))
